...
- **M07.4 adds structs + impl blocks** (see `specs/015-m07-4-structs-impl/contracts/m07-4-protocol-delta.md`): **eighth invocation** of the closed-enum-with-revisions rule. Additive variants: `Ty::Struct { name, fields: Vec<(String, Ty)> }` and `Value::Struct { name, fields: Vec<(String, Value)> }`. **First field-extension of an existing variant**: `Value::Ref` gains `field_path: Vec<String>` with `#[serde(default, skip_serializing_if = "Vec::is_empty")]` — empty when the ref points at the whole binding (M06+ semantics), non-empty when it points at a struct sub-field (`&p.x` → `vec!["x"]`). Existing M06+ borrow snapshots stay byte-identical (the new field is omitted from JSON when empty). No new `MemEvent` variants; no new `Pointee` variants — field borrows reuse `Pointee::Slot(_)`. Method calls reuse the standard `FrameEnter`/`SlotAlloc`/`SlotWrite`/`ReturnValue`/`FrameLeave` event flow with `self` as just another param slot.
- **M07.5 adds generics** (see `specs/016-m07-5-generics/contracts/m07-5-protocol-delta.md`): **ninth invocation** of the closed-enum-with-revisions rule. Additive variant: `Ty::Param(String)` carrying the unresolved type-parameter's name (e.g. `"T"`). **Second field-extension**: `Ty::Struct` gains `type_args: Vec<Ty>` with serde-default-empty — empty for non-generic structs (M07.4 byte-identical), non-empty for generic instantiations like `Wrapper<i32>` which drives `Ty::name()` to render `"Wrapper<i32>"` and drives nominal equality (`Wrapper<i32>` ≠ `Wrapper<bool>`). No new `MemEvent` variants. No new `Value` variants. No new `Pointee` variants. Monomorphization is visible via mangled `FrameEnter.fn_name`: `id::<i32>` and `id::<bool>` are distinct frames for the same source `fn id<T>(x: T)`. Substitution happens entirely in typeck (per-call-site `TypeMap.call_substs: IndexMap<Span, Vec<(String, Ty)>>` side table); eval reads pre-substituted concrete types from `binding_types` and uses `call_substs` only to build mangled fn names.
- **M07.6 adds traits** (see `specs/017-m07-6-traits/contracts/m07-6-protocol-delta.md`): **tenth invocation** of the closed-enum-with-revisions rule. **AST-only additions** — no event-protocol changes, no new `Ty`/`Value`/`Pointee` variants, no new `MemEvent` variants. New `Item::Trait(TraitDecl)` for trait declarations; `ImplBlock` extended with `trait_name: Option<String>` (None = inherent — M07.4 behavior; Some = trait impl); `TypeParam.bound: Option<String>` (M07.5 placeholder) promoted to `bounds: Vec<String>` (multi-bound support, typeck-checked). Trait dispatch frame names use the UFCS-style `<Point as Show>::show` format on `FrameEnter.fn_name` — a string convention distinct from inherent `Point::show` (M07.4) so the dispatch path is visible in the trace. Method dispatch extends M07.4's tie-breaker with a third layer (builtins → inherent → trait). Trait bounds on generics (`fn print<T: Show>(x: T)`) enable method calls on Ty::Param values inside the body — bound proves the method exists. Default methods route to the trait's body when the impl doesn't override.
- **M07.7 adds trait objects** (see `specs/018-m07-7-trait-objects/contracts/m07-7-protocol-delta.md`): **eleventh invocation** of the closed-enum-with-revisions rule. Additive `Ty` variants: `Ty::DynRef { trait_name, mutable }` and `Ty::BoxDyn { trait_name }`. Additive `Value` variants: `Value::DynRef { borrow_id, target: Pointee, vtable: VtableAddr, mutable, trait_name }` and `Value::BoxDyn { addr: HeapAddr, vtable: VtableAddr, trait_name }`. New addressing newtype `VtableAddr(u32)` — vtables are a fourth memory region after stack/heap/static, content-deduplicated by `(trait, type)`. New `MemEvent::VtableAlloc { addr, trait_name, type_name, methods, span }` — **first new `MemEvent` variant since M07.2's `StaticAlloc` + `BytesCopy`**. **No new `Pointee` variants** — vtables ride on dedicated `vtable: VtableAddr` fields, not as borrow targets. AST additions (parser-side, not in wire protocol): `Type::DynTrait { trait_name, span }` and `Expr::Cast { inner, target_ty, span }`. Method dispatch adds a fourth layer (builtins → inherent → trait → dyn): trait-object receivers dispatch through the vtable at eval time but typecheck against the trait's declared methods. Inherent methods are unreachable through dyn (rejected with a clear error). `fn print(x: &dyn Show)` produces ONE `print` frame regardless of argument type (vs M07.5's per-type monomorphization). Frame names for trait-object dispatch reuse M07.6's UFCS `<Type as Trait>::method` format; the OUTER frame name (`print` vs `print::<Point>`) is what distinguishes dynamic from static dispatch.
- **M08 adds threads + Arc + Mutex** (see `specs/019-m08-threads-arc-mutex/contracts/m08-protocol-delta.md`): **twelfth invocation** of the closed-enum-with-revisions rule. Additive `Ty` variants: `Ty::Arc(Box<Ty>)`, `Ty::Mutex(Box<Ty>)`, `Ty::MutexGuard(Box<Ty>)`, `Ty::JoinHandle`. Additive `Value` variants: `Value::Arc { addr }`, `Value::Mutex { addr }`, `Value::MutexGuard { addr }`, `Value::JoinHandle { thread_id }`. New addressing newtype `ThreadId(u32)` — analog of M07.7's `VtableAddr`; main is always `ThreadId(0)`; spawned threads get 1, 2, … in spawn order. New `MemEvent::ThreadSwitch { thread_id, span }` drives the UI's current-thread routing — eval emits whenever the cooperative scheduler picks a different thread. The 7 thread+sync MemEvent variants pre-declared at M03 (`ThreadSpawn`, `ThreadJoin`, `ThreadPark`, `ArcClone`, `ArcDrop`, `LockAcquire`, `LockRelease`) finally get their payloads emitted — variant SHAPES stay byte-identical to M03's declarations. AST addition: `Expr::Closure { is_move, body, span }` — minimal closure surface (no-arg only, body block), typeck-valid only as the `thread::spawn` argument. Single-threaded programs produce ZERO new MemEvent variants (no ThreadSwitch, no Thread*, no Arc*, no Lock*) — M01-M07.7 traces byte-identical. Eval refactored from single-stack `Vec<Frame>` to multi-thread `IndexMap<ThreadId, ThreadState>` with cooperative scheduler (rule-driven yield-points at thread::spawn / mutex.lock when held / handle.join); deterministic event stream across runs (same input → byte-identical `Vec<MemEvent>`).
